...
export function ClientFormModal({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  return (
    <SheetModal
      ...
      <Field label="Nom complet" value={name} onChangeText={setName} placeholder="Ex: Aïcha Ndongo" />
      <Field label="Téléphone" value={phone} onChangeText={setPhone} placeholder="+237 6 90 00 00 00" keyboardType="phone-pad" />
      <Field label="Email (optionnel)" value={email} onChangeText={setEmail} placeholder="exemple@domaine.com" keyboardType="email-address" />
    </SheetModal>
  );
}
